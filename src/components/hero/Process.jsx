...
export const Process = () => {
  return (
    <>
      <section className="process-section py-24 bg-primary relative">
        ...
        <Heading title="How It Works" subtitle="Easy 4 steps to win" subtitleClass="subtitle-white" />


          <div className="content grid grid-cols-1 md:grid-cols-4 gap-8 mt-8">
            {processList.map((item, index) => (
              <div key={index} className="process-card p-8 bg-[rgba(38,90,77,0.62)] rounded-xl flex items-center justify-center flex-col text-center">
                <div className="w-16 h-16">
                  <img src={item.cover} alt={item.title} />
                </div>
                <Title level={5} className="title my-3 font-normal text-white">
                  {item.title}
                </Title>
